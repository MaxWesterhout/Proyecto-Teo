# -*- coding: utf-8 -*-
"""RESNET18-KNN-1024DATOS.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1SbWvMh4O8Gmyd40fkDZIn1v1219VnLJE
"""
...
import glob
idxs = np.random.choice(np.arange(len(subset_train_dataset)),1024, replace = False) #Se sacan 500 imagenes
...
#Valores por clase
df_depth_clase['Descripcion'].value_counts()
...
#Pasamos gpu
trainloader = [(datos.to(device), etiquetas.to(device)) for datos, etiquetas in subset_trainloader]

del train_dataset
del test_dataset

#print(model)
#Falta modificar la capa de salida
#ALFINAL DESTRUI EL MODELO AGREGANDO UNA CAPA ABAJO, LA ULTIMA QUE TERMINA EN 10
#USE ESTA LINEA DE CODIGO model.model.fc = nn.Linear(512, 10)
#Depuest tuve que usar model.device() denuevo por que no reconocia el modelo, de todos mods dio 0 el accuracy con este metodo
...
# Calcular precisión en el conjunto de datos CIFAR-10
model = model.eval()
accuracies = []
...
with torch.no_grad():
    for inputs, labels in trainloader:
        ...
        # Asumiendo que 'fc' es la última capa para la clasificación
        logits = outputs['fc']
...
print(f'Precisión media en CIFAR-10: {mean_accuracy * 100:.2f}%')
'''
...
df_capas.to_csv('Dataframes_capas_pd_RESNET18-1-1024DATOS.csv', index=False)