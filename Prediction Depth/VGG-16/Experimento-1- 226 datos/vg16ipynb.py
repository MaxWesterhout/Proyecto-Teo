...
for i in range(0,10):
  ...
  dict_indices[clase] = []
  if i in [0, 1,2,3]:
    dict_numero_imagenes[clase] = 23
  else:
    dict_numero_imagenes[clase] = 22
...
# Número de muestras
num_samples = 224 #Funciona correctamente con 96
...
#Ojo que ese images_per_row es solo para este caso
imshow_grid([subset_data[x][0] for x in idx[:largo]],[f'N:{x}, Class:{descripciones[x]} , Prediccion KNN:{CIFAR10_LABEL_NAMES[vector_predicciones[x]]}'for x in idx[:largo]],
title = f'Imagenes con Prediction Depth = {7}', images_per_row = 1)
...
#Ojo que ese images_per_row es solo para este caso
imshow_grid([subset_data[x][0] for x in idx[:largo]],[f'N:{x}, Class:{descripciones[x]} , Prediccion KNN:{CIFAR10_LABEL_NAMES[vector_predicciones[x]]}'for x in idx[:largo]],
title = f'Imagenes con Prediction Depth = {4}', images_per_row = 2)
...
#Ojo que ese images_per_row es solo para este caso
imshow_grid([subset_data[x][0] for x in idx[:largo]],[f'N:{x}, Class:{descripciones[x]} , Prediccion KNN:{CIFAR10_LABEL_NAMES[vector_predicciones[x]]}'for x in idx[:largo]],
title = f'Imagenes con Prediction Depth = {3}', images_per_row = 1)
...
df_clases_cada_pd.to_csv('df_clases_cada_pd_VGG16-2-224DATOS-equilibrados.csv', index=False)
df_capas.to_csv('Dataframes_capas_pd__VGG16-1-224DATOS-equilibrados.csv', index=False)